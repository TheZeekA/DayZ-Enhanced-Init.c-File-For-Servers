...
*/				
  ...
  override void OnInit()
	...
	void sendWelcomeMessage(PlayerBase player)
	{
		...
		string welcomeMsg = "[[Welcome]] " + player.GetIdentity().GetName() + "! Players Online: " + playerCount;
		sendPlayerMessage(player, welcomeMsg);
	}
	...
	override void InvokeOnConnect(PlayerBase player, PlayerIdentity identity)
	{
		...
		GetGame().GetCallQueue(CALL_CATEGORY_GAMEPLAY).CallLater(sendWelcomeMessage, 5000, false, player); //made this change
